...
Mass is a fundamental property of matter that measures the amount of matter in an object. It is a scalar quantity and is typically denoted by the symbol $m$, and its standard unit is the kilogram $kg$.

Mass can be accelerated or decelerated by a force. The unit of force, the newton $N$, is defined such that 1 newton acting on 1 kilogram of mass results in an acceleration of 1 meter per second squared. This leads to the equation:
...
In physics, **work** is defined as the transfer of energy that occurs when a force is applied to an object, and the object is displaced in the direction of the force. Mathematically, work is expressed as:
...
- \( \vec{d} \) is the displacement vector (in meters, \( \text{m} \)).
- \( \theta \) is the angle between the force and the displacement vectors.

## Key Points

1. **Work is a scalar quantity:** It has magnitude but no direction.
2. **Positive Work:** Occurs when the force has a component in the direction of displacement (\( 0^\circ \leq \theta < 90^\circ \)).
3. **Negative Work:** Happens when the force opposes the direction of displacement (\( 90^\circ < \theta \leq 180^\circ \)).
4. **Zero Work:** If the displacement is zero or the force is perpendicular to the displacement (\( \theta = 90^\circ \)).

## Special Cases

- If the force is constant and parallel to the displacement (\( \theta = 0^\circ \)), the work simplifies to:

$$
W = Fd
$$

- If there is no displacement or no force acting on the object, then:
...
Read Chapter 1 in @russell2022 and complete the end of chapter problems.